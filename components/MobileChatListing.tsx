...
const MobileChatListing: React.FC<{ activeItem?: string }> = (activeItem) => {
   ...
   // useEffect(() => {
   //    Chat.ListenForLastReadMessages("mobile-listing", (chatMessages) => {
   //       setLoading(false)
   //       setMessages(chatMessages);
   //       let unread = 0;
   ...
   //    })
   //    return () => {
   //       Chat.StopListeningForLastReadMessages("mobile-listing")
   //    }
   // }, [])
...
<div className="sidechat-top-bnts">
   ...
   <Link onClick={() => {DrawerComponent.hideAll()}} to={`/landing-pub`}>
      <IconButton color="error">
         <i className="fas fa-home"></i>
      </IconButton>
   </Link>

   {!isPage && !messagingPanel && <Link onClick={() => {DrawerComponent.hideAll()}} to={`/favorites`}>
      <IconButton color="error" className="d-flex d-sm-none">
         <i className="fa fa-heart"></i>
      </IconButton>
   </Link>}

   <Link onClick={() => {DrawerComponent.hideAll()}} to={'javascript:void()'} data-bs-toggle="modal" data-bs-target="#kt_modal_1" className="d-block d-sm-none">
      <IconButton color="error" className='dollar-icon'>
         <i className="material-icons">monetization_on</i>
      </IconButton>
   </Link>

   <Link onClick={() => {DrawerComponent.hideAll()}} to={`/my-profile`} className="d-block d-sm-none" data-kt-drawer-close="#kt_drawer_example_basic_close">
      <IconButton color="error">
         <i className="fa fa-user"></i>
               ...
               {loader && <div className="spinnerLoder"><CircularProgress /></div>}
                  ...
                  <div className='chat-nav-list custom-scroll'>
                     {messages?.map((item, index) => {
                        ...
                        return (
                           ...
                           <List component="nav" key={index} className={(activeItem.activeItem === item.profile_id) ? 'chatprofile-grid active ' : 'chatprofile-grid'} onClick={() => { onMessageClicked(item.profile_id);DrawerComponent.hideAll() }}>
                             {
...
export default MobileChatListing;